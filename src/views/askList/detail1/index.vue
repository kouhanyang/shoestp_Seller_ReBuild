...
v-html="chartContent" @click="focusInput">
          ...
          <div class="opearte-row">
            <div>
              已抢单 {{ supplierCount }}/5 仅剩{{ 5-supplierCount }}
            </div>
            <div>
              报价：
              <el-input v-model="test" type="number" min="0" placeholder="请输入报价" @input="validTest"/>
              $
            </div>
            <div>
...
import chartInquiryInfo from '../../belongasklist/detailprivate/components/chart-inquiry-info'
import chartWindow from '../../belongasklist/detailprivate/components/chart-window'
...
export default {
  ...
  methods: {
    validTest: function() {
      if (isNaN(this.test) || Number(this.test) < 0) {
        this.test = 0
      }
    },
    // 点击使textarea聚焦},
    ...
    loadinquiry() {
      ...
      self.supplierCount = this.$router.history.current.params.supplierCount
    ...
    sendMsg: function() {
      ...
      if (self.supplierCount === 5) {
        ...
        var textImgs = ''
        //  textImgs = (self.imgsToUpload && self.imgsToUpload.length) ? (self.myImgLabelBefore + self.imgsToUpload.join(',') + self.myImgLabelAfter) : ''
        if (self.imgsToUpload && self.imgsToUpload.length) {
          for (var i = 0; i < self.imgsToUpload.length; i++) {
            console.log(self.imgsToUpload[i])
